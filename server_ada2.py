    ...
    if not os.path.isdir(ada2_client_path):
        ...
        def _inject_subtitle_script(html: str) -> str:
            ...
            injection = (
                ...
                "        const box = getBox();\n"
                "        if(msg.type==='subtitle_start'){ box.textContent=''; box.style.display='-webkit-box'; }\n"
                "        else if(msg.type==='subtitle_delta'){ box.textContent = (msg.text||''); box.style.display='-webkit-box'; }\n"
                "        else if(msg.type==='subtitle_end'){ box.textContent = (msg.text||''); box.style.display = (box.textContent?'-webkit-box':'none'); }\n"
                "      }catch(e){}\n"
                "    };\n"
