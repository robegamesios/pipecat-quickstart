    ...
    if not os.path.isdir(ada2_client_path):
        ...
        def _inject_subtitle_script(html: str) -> str:
            ...
            injection = (
                ...
                "        case 'log_tool':\n"
                "          try{ const phase = (msg.phase||''); if(phase==='llm_only'){ /* ignore: no Local LLM display */ break; } const name = (msg.name||''); let label='Tool'; if(name==='google_search'){ label='Google Search'; } else if(name==='get_current_weather'){ label='Weather Search'; } __toolActive=true; holdLabel(label); }catch(e){}\n"
                "          break;\n"
