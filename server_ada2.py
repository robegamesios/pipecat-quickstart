    ...
    if not os.path.isdir(ada2_client_path):
        ...
        def _inject_subtitle_script(html: str) -> str:
            ...
            injection = (
                ...
                "      el.style.maxWidth = 'calc(100% - 80px)';\n"
                "      el.style.zIndex = 200;\n"
                "    }catch(e){}\n"
                "  }\n"
                "  function getBox(){\n"
                "    let el = document.getElementById('subtitles');\n"
                "    if(!el){ el = document.createElement('div'); el.id='subtitles'; el.style.padding='8px 12px'; el.style.background='rgba(0,0,0,0.6)'; el.style.borderRadius='10px'; el.style.fontSize='18px'; el.style.lineHeight='1.25'; el.style.maxHeight='3.75em'; el.style.textAlign='center'; el.style.color='#fff'; el.style.pointerEvents='none'; el.style.overflow='hidden'; el.style.display='none'; el.style.webkitLineClamp=3; el.style.webkitBoxOrient='vertical'; var parent=(document.getElementById('avatar-container')||document.body); parent.appendChild(el);}\n"
                "    __centerOverAvatar(el);\n"
                "    return el;\n"
                "  }\n"
                "  // Continuously recenter subtitles during speech to survive layout transitions\n"
                "  function startSubtitleRecenter(){\n"
                "    if (window.__subtitleRecenterActive) return;\n"
                "    window.__subtitleRecenterActive = true;\n"
                "    let last = 0;\n"
                "    const step = (ts)=>{\n"
                "      if (!window.__subtitleRecenterActive) return;\n"
                "      if (ts - last > 33){ const box = document.getElementById('subtitles'); if (box) __centerOverAvatar(box); last = ts; }\n"
                "      requestAnimationFrame(step);\n"
                "    };\n"
                "    requestAnimationFrame(step);\n"
                "  }\n"
                "  function stopSubtitleRecenter(){ window.__subtitleRecenterActive = false; }\n"
                ...
                "      try{ window.addEventListener('resize', ()=>{ const box = document.getElementById('subtitles'); if(box) __centerOverAvatar(box); }, { passive:true }); }catch(_){}\n"
                ...
                "      const box = getBox();\n"
                "      switch(msg && msg.type){\n"
                "        case 'subtitle_start': /*console.log('[Ada2DC] subtitle_start');*/ __centerOverAvatar(box); box.textContent=''; box.style.display='-webkit-box'; __speakingActive=true; startSubtitleRecenter(); /* no overlay change here to avoid flicker */ break;\n"
                ...
                "        case 'subtitle_end': /*console.log('[Ada2DC] subtitle_end');*/ __centerOverAvatar(box); box.textContent = (msg.text||''); box.style.display = (box.textContent?'-webkit-box':'none'); __speakingActive=false; stopSubtitleRecenter(); releaseLabel(); __toolActive=false; break;\n"
                ...
                "      // Keep voice chat connected if present; avoid starting a competing reader RTC\n"
                "      try{ if(window.avatarVoiceChat && window.avatarVoiceChat.isConnected && window.readerRTC && window.readerRTC.disconnect){ window.readerRTC.disconnect(); } }catch(_){ }\n"
                "      // If voice chat is not connected, ensure the lightweight reader channel is connected for document reading\n"
                "      try{ var vcOn = !!(window.avatarVoiceChat && window.avatarVoiceChat.isConnected); if(!vcOn && window.readerRTC && window.readerRTC.connect){ await window.readerRTC.connect(); } }catch(_){ }\n"
