    ...
    if not os.path.isdir(ada2_client_path):
        ...
        def _inject_subtitle_script(html: str) -> str:
            ...
            injection = (
                ...
                "            // Build approximate word timings from total duration\n"
                "            const text = (meta.text||'').trim(); const words = text ? text.split(/\s+/) : [];\n"
                ...
                "            const n = Math.max(1, words.length); const lens = words.map(w=>Math.max(1,w.length)); const sum = lens.reduce((a,b)=>a+b,0);\n"
                "            let acc=0; const wtimes = []; const wdurations = [];\n"
                "            for(let i=0;i<n;i++){ const dur = Math.max(80, Math.round(totalMs * (lens[i]/sum))); wtimes.push(acc); wdurations.push(i===n-1 ? Math.max(120, totalMs-acc) : dur); acc += dur; }\n"
